...
var drawPixel = function(snapshot){
	...
	if (myPort.options.open) {
		myPort.write("micadena\n"+coords[0]);
		myPort.write("micadena\t"+coords[1]);
		//console.log(hexToRgb(snapshot.val()));
		var r = hexToRgb(snapshot.val()).r;
		var g = hexToRgb(snapshot.val()).g;
		var b = hexToRgb(snapshot.val()).b;
		myPort.write("micadena\f"+r);
		myPort.write("micadena\\"+g);
		myPort.write("micadena\b"+b);
	}
...
var clearPixel = function(snapshot) {
  var coords = snapshot.name().split(":");
	if (myPort.options.open) {
		...
		myPort.write("micadena\f"+0);
		myPort.write("micadena\\"+0);
		myPort.write("micadena\b"+0);
	}
};
